...
1. **Hypothesis decomposition.** Label competing mechanisms explicitly (H1, H2a, H2b) and adjudicate each with evidence. Structure: "Why does X happen? → H1: [mechanism]? No: [evidence against]. → H2: [mechanism]? Yes: [evidence for]."
...
Check whether the project has `beamerthemeCapeTown.sty` in its slides directory. If not, search sibling project directories under `~/Dropbox/` for a copy. If unavailable, use the Metropolis theme (`\usetheme{metropolis}`) as fallback.
...
**Calibration decks (primary source):**
Add 3–5 of your own best slide decks as calibration examples — study them for structure, not topic. For each, note what it does well: research-question-as-structure, hypothesis decomposition, prior-before-result framing, a clean interpretive model. Place the decks in your personal config (e.g., `<your-sync-folder>/skills-overrides/slides/examples/`), not in this shared repo. The skill works without examples but produces sharper output with them.

Read at least one full deck and, where available, a few excerpted frames before composing slides, to calibrate register and density. Full decks show how the rhetorical moves sequence across a talk; excerpts show individual frame quality.
