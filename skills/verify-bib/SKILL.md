...
3a. **Library proxy fallback (WebFetch)**. If step 2 finds only paywalled or access-gated URLs, before flagging as paywalled, attempt to fetch through the user's institutional library proxy *if they have configured a proxy prefix* (see note at the end of this step): prepend that prefix to the direct URL and try `WebFetch`. If the response contains the expected PDF (check content-type and first-page text), save and proceed as in steps 2-3. If the response is a login/CAS redirect page or HTML, `WebFetch` can't authenticate — continue to step 3b.
...
4. **Add a silent comment** to the bib entry:
   ...
   ```
5. **If only paywalled versions exist** (publisher requires subscription, only paid preview available, Sci-Hub/LibGen should NOT be used): add to `<project>/.workspace/bib_paywalled_todos.md` a line with both the direct and library-proxied URLs:
   ```
   - [ ] Author (2020) — manual download needed.
     - Direct: https://journal.com/article/...
     - Proxied: `<proxy-prefix>` + https://journal.com/article/... (e.g., a prefix of the form `https://proxy.library.<institution>.edu/login?url=`)
   ```
   The proxy prefix routes the request through the institution's library authentication; when the user logs in with their institutional credentials they can download the PDF. Set your proxy prefix in your personal config (many universities use the form `https://proxy.library.<institution>.edu/login?url=`); if no proxy is configured, skip this fallback. Always construct the proxied URL by prepending the prefix to the direct URL — don't URL-encode the inner URL, the proxy accepts it raw.
